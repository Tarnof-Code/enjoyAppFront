...
| Réunions | `GET /sejours/{sejourId}/reunions` | `Home` (dernière réunion par défaut ; choix via modale) |
...
| `sejour-reunion.service.ts` | Réunions séjour (liste accueil + choix modale) |
...
| `reunionAccueil.ts`, `reunionTipTapTexte.ts` | Accueil réunions (`Home`) — **`trierReunionsPlusRecentVersAncien`**, **`trouverDerniereReunion`**, **`formatTitreCompteRenduAccueil`**, **`formatDateReunionListe`**, **`estContenuTipTapVide`** |
...
| `CompteRenduPleinEcranModal.tsx` | Lecture plein écran réunion sélectionnée (ordre du jour + TipTap) |
...
- **Réunion accueil** : dernière réunion du séjour par défaut sur `Home` (tri date ↓ puis `id` ↓) ; tap titre → modale liste (date + ordre du jour) ; sélection conservée au refresh ; titre **`formatTitreCompteRenduAccueil`** ; contenu **`ReunionContenuTipTapJson`** via **`ReunionContenuTipTap`**.
