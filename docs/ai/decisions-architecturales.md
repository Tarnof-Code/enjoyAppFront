...
- **Cas particuliers** : `Animators` lit le store Redux (`sejourCourant`) ; charge en parallèle groupes, chambres et profil directeur (tél./e-mail, **`photoProfilUrl`**) ; photos équipe via **`usePhotosProfilEquipe`** + **`AvatarProfil`** sur cartes et modale ; tri équipe + modal détail via **`FichePersonneModal`** (`photoUri`, zoom). `Children` : chargement parallèle enfants/groupes/chambres/dossiers + refresh séjour ; dates séjour pour anniversaire. `Bedrooms` : chambres + groupes + enfants en parallèle ; filtres locaux ; **CRUD chambres et occupants** (modales dédiées, mise à jour liste locale via `fusionnerChambreRetourneeDansListe`). `Home` gère son propre refresh (CR veille + photo + liste séjours).
...
- **Affichage photo ailleurs** : **`Header`** et **`Home`** lisent **`auth.photoProfilUri`** ; liste **Équipe** — **`usePhotosProfilEquipe`** + **`AvatarProfil`** (cartes + modale) ; tap → **`Profil`** (connecté) ou zoom (autres membres) ; refresh global **`rafraichirPhotoProfil`** au pull-to-refresh.
