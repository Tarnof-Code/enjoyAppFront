...
- **Cas particuliers** : `Animators` lit le store Redux (`sejourCourant`) ; charge en parallèle groupes, chambres et profil directeur (tél./e-mail, **`photoProfilUrl`**) ; photos équipe via **`usePhotosProfilEquipe`** + **`AvatarProfil`** sur cartes et modale ; tri équipe + modal détail via **`FichePersonneModal`** (`photoUri`, zoom). `Children` : chargement parallèle enfants/groupes/chambres/dossiers + refresh séjour ; dates séjour pour anniversaire. `Bedrooms` : chambres + groupes + enfants en parallèle ; filtres locaux ; **CRUD chambres et occupants** (modales dédiées, mise à jour liste locale via `fusionnerChambreRetourneeDansListe`). **`Home`** : refresh dédié (réunions veille + **`rafraichirPhotoProfil`** + liste séjours) ; affichage CR via JSON TipTap (**`ReunionContenuTipTap`**) et modale plein écran ; modales (séjour, CR) fermées si écran non focus.
