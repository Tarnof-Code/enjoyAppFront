...
- **Build EAS (Android APK)** : org Expo **`just-enjoy-app`** (`app.json` **`owner`**) ; **`extra.eas.projectId`** dans **`app.json`**. **`eas.json`** — profil **`development`** (dev client, internal), **`preview`** (APK Android, distribution internal, **`ENJOY_ENV=prod`** au build cloud), **`production`**. Script **`npm run build:apk`** → `eas build --platform android --profile preview`. Au build preview, **`app.config.js`** appelle **`loadEnjoyEnv('prod')`** → **`EXPO_PUBLIC_API_URL`** depuis **`.env.prod`** (fichier non versionné, à configurer sur la machine locale et sur EAS via secrets/env si besoin).
...
- **Cas particuliers** : `Animators` lit le store Redux (`sejourCourant`) ; charge en parallèle groupes, chambres et profil directeur (tél./e-mail, **`photoProfilUrl`**) ; photos équipe via **`usePhotosProfilEquipe`** + **`AvatarProfil`** sur cartes et modale ; tri équipe + modal détail via **`FichePersonneModal`** (`photoUri`, zoom) ; pull-to-refresh dédié recharge séjour + photo (`getSejourById`, **`rafraichirPhotoProfil`**). `Children` : chargement parallèle enfants/groupes/chambres/dossiers ; dates séjour pour anniversaire. `Bedrooms` : chambres + groupes + enfants en parallèle ; filtres locaux ; **CRUD chambres et occupants** (modales dédiées, mise à jour liste locale via `fusionnerChambreRetourneeDansListe`). **`Home`** : sans `sejourCourant` — pas de chargement réunion, invite choix séjour, onglets restreints ; avec séjour — liste réunions API, affichage **dernière réunion** par défaut, **modale choix** au tap titre (sélection mémorisée au refresh via ref) ; refresh réunions + **`rafraichirPhotoProfil`** + liste séjours ; CR TipTap + modale plein écran ; modales fermées si écran non focus.
