...
Rappels essentiels : réponses en français ; `npm` + `legacy-peer-deps` ; secrets interdits ; types dans `types/api.d.ts` (alignés web) ; **`tokenId`** jamais id SQL ; client **`services/httpClient.ts`** ; auth SecureStore + refresh single-flight (`X-Skip-Token-Refresh` chaîne `'true'`). **Navigation** : Stack `Login` → `SejourPicker` → **6 onglets** ; top-tabs **`creerTopTab`**. **Orga / Menus / Activités** : grilles calendrier 1/3/5 j., **`useFenetreJoursPlanning`**, paysage tableau (**`ConteneurGrillePaysage`**) ; **Orga + Activités** : en-tête dates fixe (**`EnteteJoursGrille`**, grille bord à bord). **Activités** : fusion sorties + CRUD modales + conflits directeur ; helpers **`activiteUtils`**, **`activitePrestataireCalendrier`**, **`construireArbreMoments`**. **Sorties (liste)** : accordéons + **`PUT …/enfants`** + **`SortieEnfantsParticipantsModal`**. **Plannings orga** : édition cellules **`PlanningCelluleModal`**, droits **par ligne**. **Listes** : **`FichePersonneModal`**, **`ListeAccordion`**, tri **`triListesSejour`**.

Mise à jour mémoire : commande **`/maj`**. Dernière MAJ : **2026-06-26** (correction typecheck modal Sorties + httpClient).
