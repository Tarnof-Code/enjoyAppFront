...
Rappels essentiels : réponses en français ; `npm` + `legacy-peer-deps` ; secrets interdits ; types dans `types/api.d.ts` (alignés web) ; **`tokenId`** jamais id SQL ; client **`services/httpClient.ts`** ; auth SecureStore + refresh single-flight. **Navigation** : Stack `Login` → `SejourPicker` → **6 onglets** ; top-tabs **`creerTopTab`** (Listes, Activités, **Sanitaire**). **Sanitaire** : **`TopTabSanitaire`** — **Cahier d'infirmerie** (CRUD, filtre jour = dates avec entrées + recherche, **`CahierInfirmerieFormModal`**, date/heure séparées `@react-native-community/datetimepicker`) + **Dossiers sanitaires** (lecture, **`Dropdown`** Tout/Traitements/Alimentation/Médical/À prendre en sortie/Autres infos ; sous-filtre moment si Traitements). **Dates API** : **`parseDateDepuisValeurApi`** / **`dayjsDepuisValeurApi`** (epoch s ou ms). **Orga / Menus / Activités** : grilles calendrier 1/3/5 j., **`useFenetreJoursPlanning`**, paysage tableau ; **Sorties** : accordéons + **`PUT …/enfants`**. **Listes** : **`FichePersonneModal`**, **`ListeAccordion`**, tri **`triListesSejour`**.

Mise à jour mémoire : commande **`/maj`**. Dernière MAJ : **2026-06-26** (Dossiers sanitaires — filtres déroulants + sous-filtre traitements).
