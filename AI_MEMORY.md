...
Rappels essentiels : réponses en français ; `npm` + `legacy-peer-deps` ; secrets interdits ; types dans `types/api.d.ts` (alignés web) ; **`tokenId`** jamais id SQL ; client **`services/httpClient.ts`** ; auth SecureStore + refresh single-flight. **Navigation** : Stack `Login` → `SejourPicker` → **6 onglets** ; top-tabs **`creerTopTab`**. **Orga / Menus / Activités** : grilles calendrier 1/3/5 j., **`useFenetreJoursPlanning`**, paysage tableau (**`ConteneurGrillePaysage`**). **Activités** : fusion sorties + CRUD modales + conflits directeur ; helpers **`activiteUtils`**, **`activitePrestataireCalendrier`**, **`construireArbreMoments`** ; libellés animateurs **`libelleMembreDansCelluleEquipe`**. **Plannings orga** : édition cellules **`PlanningCelluleModal`**, droits **par ligne**. **Listes** : **`FichePersonneModal`**, **`ListeAccordion`**, tri **`triListesSejour`**.

Mise à jour mémoire : commande **`/maj`**. Dernière MAJ : **2026-06-25** (calendrier Activités : grille, sorties, CRUD, filtres, conflits).
