...
Rappels essentiels : réponses en français ; `npm` + `legacy-peer-deps` ; secrets interdits ; types dans `types/api.d.ts` (alignés web) ; **`tokenId`** jamais id SQL ; client **`services/httpClient.ts`** ; auth SecureStore + refresh single-flight. **Navigation** : Stack `Login` → `SejourPicker` → **6 onglets** ; top-tabs **`creerTopTab`**. **Données** : API Enjoy uniquement ; **`useChargementRafraichissable`** + pull-to-refresh ; **`useRafraichirSejourCourant`** pour resync séjour. **Listes** : modal **`FichePersonneModal`** (Équipe, Enfants) ; accordéons **`ListeAccordion`** (Groupes, Chambres) ; tri/libellés via **`triListesSejour`**. **Chambres (écriture)** : **`chambre.service`** CRUD + occupants ; modales **`ChambreFormulaireModal`** / **`AffecterOccupantsModal`**. **Organisation / plannings** : liste tri alpha + recherche titre ; détail matrice multi-jours (1/3/5) ; édition cellules via **`PlanningCelluleModal`** ; helpers **`planningGrilleUtils`** ; écriture **`planningGrille.service`** (PUT cellules, PATCH ma-presence).

Mise à jour mémoire : commande **`/maj`**. Dernière MAJ : **2026-06-25** (refonte UX plannings organisation mobile).
