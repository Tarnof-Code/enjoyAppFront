...
Mise à jour mémoire : commande **`/maj`**. Dernière MAJ : **2026-06-27** (écran Mon profil, photo Redux, recadrage/zoom).

Rappels récents (détail dans `docs/ai/`) : **`Profil`** (Stack) — édition alignée web, **`ChangePasswordModal`**, modales photo **`PhotoProfilRecadrageModal`** / **`PhotoProfilZoomModal`** ; avatar API **`Header`** / **`Home`** ; onglets actifs / header en **`colors.primary`** ; **Activités** → sous-onglet **Planning** (icône seule `calendar-blank`) ; grilles **`BoutonModePaysageGrille`** à droite ; **`EnteteJoursGrille`** prop **`compact`** sur **`Activites`**.
